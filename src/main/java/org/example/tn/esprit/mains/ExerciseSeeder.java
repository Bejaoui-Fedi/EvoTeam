...
public class ExerciseSeeder {
    ...
    public static void main(String[] args) {
        ...
        User admin = new User();
        admin.setRole("ADMIN");
        admin.setId(1); // Assuming ID 1 exists or doesn't matter for the check
        ...
        try {
            // 2. Fetch Objectives to link to
            List<Objective> objectives = serviceObjective.show();
            if (objectives.isEmpty()) {
                System.err.println("No objectives found! Please run ObjectiveSeeder first.");
                return;
            }

            // We need at least 3 objectives ideally, but we'll work with what we have
            ...
            for (Exercise ex : exercises) {
                try {
                    ...
                    System.out.println("Inserted Exercise: " + ex.getTitle() + " -> Obj ID: " + ex.getObjectiveId());
                } catch (Exception e) {
                    System.err.println("Error inserting " + ex.getTitle() + ": " + e.getMessage());
                }
            }
            System.out.println("Exercise seeding completed.");
