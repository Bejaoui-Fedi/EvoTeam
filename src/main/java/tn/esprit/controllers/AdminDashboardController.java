...
import tn.esprit.controllers.admin.AdminSupervisionRdvController;
...
public class AdminDashboardController implements Initializable {
    ...
    private void handleAppointments() {
        ...
        try {
            URL fxmlUrl = getClass().getResource("/fxml/admin/admin_supervision_rdv.fxml");
            if (fxmlUrl == null) {
                System.err.println("❌ Fichier FXML introuvable : /fxml/admin/admin_supervision_rdv.fxml");
                showError("Fichier admin_supervision_rdv.fxml introuvable !");
                return;
            }
            ...
            // Passer la référence du dashboard au controller des rendez-vous
            Object controller = loader.getController();
            if (controller instanceof AdminSupervisionRdvController) {
                ((AdminSupervisionRdvController) controller).setDashboardController(this);
            }

            contentArea.getChildren().clear();
            contentArea.getChildren().add(rdvView);

            System.out.println("✅ Vue des rendez-vous chargée avec succès");

        } catch (IOException e) {
            System.err.println("❌ Erreur détaillée: " + e.getMessage());
            e.printStackTrace();
            showError("Impossible de charger la page des rendez-vous : " + e.getMessage());
        }
    }
