    ...
    private void goToAddReview() {
        ...
        try {
            ...
            if (controller instanceof AddReview) {
                ...
                // Pre-remplir l'eventId pour que l'user n'ait pas a le choisir
                if (currentEvent != null) {
                    addReviewCtrl.setEventId(currentEvent.getEventId());
                }
                // Passer le userId pour qu'il soit insere dans la review
                if (currentUser != null) {
                    addReviewCtrl.setUserId(currentUser.getId());
                }
            }
    ...
    private void confirmAndDelete(Review r) {
        ...
        confirm.showAndWait().ifPresent(resp -> {
            if (resp == ButtonType.OK) {
                serviceReview.delete(r.getReviewId());  // ← Appelle la méthode delete() de ton service
                loadCards();  // Recharge la liste après suppression
