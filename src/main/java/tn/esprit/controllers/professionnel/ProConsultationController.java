...
public class ProConsultationController {
    ...
    public void setCurrentUser(User user) {
        ...
        if (user != null) {
            System.out.println("✅ Utilisateur connecté dans ProConsultationController: " +
                    user.getNom() + " (" + user.getRole() + ")");
        }
    }

    // ✅ Méthode pour recevoir le dashboard controller
    public void setDashboardController(UserDashboardController dashboardController) {
        this.dashboardController = dashboardController;
        System.out.println("✅ Dashboard controller reçu dans ProConsultationController");
    }
            ...
            protected void updateItem(String statut, boolean empty) {
                ...
                if (empty || statut == null) {
                    ...
                    if (lowerStatut.equals("en cours")) {
                        setStyle("-fx-background-color: #d1ecf1; -fx-text-fill: #0c5460; -fx-font-weight: bold; -fx-alignment: CENTER;");
    ...
    private void setupSpinner() {
        dureeSpinner.setValueFactory(new SpinnerValueFactory.IntegerSpinnerValueFactory(1, 120, 30));
        dureeSpinner.setEditable(true);
    }

    private void setupChoiceBoxes() {
        statutConsultationChoice.setItems(FXCollections.observableArrayList("attente", "en cours", "clôturée", "annulée"));
        ...
        try {
            ...
            rdvCombo.setConverter(new javafx.util.StringConverter<Appointment>() {
                ...
                public String toString(Appointment a) {
                    if (a == null) return "";
                    return String.format("RDV #%d - %s %s - Patient #%d",
                            a.getId(), a.getDateRdv(), a.getHeureRdv(), a.getUserId());
                }
            ...
            showAlert(Alert.AlertType.ERROR, "Erreur", "Impossible de charger les rendez-vous : " + e.getMessage());
        }
    }
    ...
    private void loadConsultations() {
        try {
            ...
            showAlert(Alert.AlertType.ERROR, "Erreur", "Impossible de charger les consultations : " + e.getMessage());
        }
    }
    ...
    private void handleSave() {
        ...
        if (selectedRdv == null) {
            showAlert(Alert.AlertType.WARNING, "Validation", "Veuillez sélectionner un rendez-vous");
            return;
        }
        ...
        if (exists) {
            showAlert(Alert.AlertType.WARNING, "Attention", "Une consultation existe déjà pour ce rendez-vous");
            return;
        }
    ...
    private void handleUpdate() {
        if (selectedConsultation == null) {
            showAlert(Alert.AlertType.WARNING, "Attention", "Veuillez sélectionner une consultation à modifier.");
            return;
        }
    ...
    private void handleDelete() {
        ...
        confirm.setContentText("Êtes-vous sûr de vouloir supprimer cette consultation ?\nCette action est irréversible !");
    ...
    private boolean validateInputs() {
        if (rdvCombo.getValue() == null) { messageLabel.setText("❌ Sélectionnez un rendez-vous"); return false; }
        if (diagnosticField.getText().trim().isEmpty()) { messageLabel.setText("❌ Le diagnostic est obligatoire"); return false; }
        if (statutConsultationChoice.getValue() == null) { messageLabel.setText("❌ Sélectionnez un statut"); return false; }
        messageLabel.setText("");
        return true;
    }
    ...
    private void retourGestionRdv() {
        if (dashboardController != null) {
            // Utiliser le dashboard pour retourner aux rendez-vous
            try {
                ...
                // CORRECTION: Utiliser setContent() au lieu de getContentArea()
                dashboardController.setContent(rdvView);

                System.out.println("✅ Retour aux rendez-vous via dashboard");
            } catch (IOException e) {
                showAlert(Alert.AlertType.ERROR, "Erreur", "Impossible de retourner à la gestion des rendez-vous: " + e.getMessage());
            }
        } else {
            // Fallback si pas de dashboard controller
            try {
                FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/professionnel/pro_gestion_rdv.fxml"));
                Parent root = loader.load();
