...
public class ServiceReview {
    ...
    // ================== GET ALL REVIEWS WITH USER NAMES (NOUVEAU) ==================
    public List<Review> getAllWithUserNames() {
        List<Review> reviews = new ArrayList<>();
        // Jointure SQL pour récupérer le nom de l'utilisateur
        String sql = "SELECT r.*, u.nom as userName FROM review r " +
        ...
        try (PreparedStatement pst = connection.prepareStatement(sql);
            ...
            while (rs.next()) {
                Review review = new Review(
                        ...
                        rs.getInt("eventId"),
                        rs.getInt("userId"),
                        rs.getString("userName")  // ← Le nom récupéré
                );
                reviews.add(review);
            }
        } catch (SQLException e) {
    ...
    // ================== SEARCH REVIEWS WITH USER NAMES (NOUVEAU) ==================
    public List<Review> searchWithUserNames(String searchText) {
        List<Review> reviews = new ArrayList<>();
        String sql = "SELECT r.*, u.nom as userName FROM review r " +
                "LEFT JOIN user u ON r.userId = u.id " +
                "WHERE r.title LIKE ? OR r.comment LIKE ? OR u.nom LIKE ?";
    ...
    // ================== VERIFIER SI UNE REVIEW EXISTE DEJA ==================
